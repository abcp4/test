...
def make_model(n_classes, alpha=1, seed=12345, n_blocks=4, width=99, height=99, n_channels=1) :
	...
	input = x
	# input = tf.layers.max_pooling2d( inputs = x, pool_size = [5, 5], strides = beta, padding = 'same' )

	for n_block in range( n_blocks ) :
		block_conv, block_res = am.blockA( input, initializer, num_maps1=2**(n_block)*alpha, num_maps2=2**(n_block+1)*alpha )
