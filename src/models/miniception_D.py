...
model_name = 'miniception_D'
model_description = """inception network architecture"""

def make_model( n_classes, alpha=1, beta = 1, seed=12345, n_blocks=4, width=99, height=99, n_channels=1):
    ...
    output_logits = tf.contrib.layers.fully_connected(fc1, num_outputs=n_classes, weights_initializer = initializer,
                                activation_fn=None)
...
def make_model_loss(y, lr_placeholder, output_logits) :
    with tf.variable_scope( 'Train' ) :
        with tf.variable_scope( 'Loss' ) :
            loss = tf.reduce_mean( tf.nn.sparse_softmax_cross_entropy_with_logits( labels = y, logits = output_logits ),
                                   name = 'loss' )
